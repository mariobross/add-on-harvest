...
class Wo extends CI_Controller{
    ...
    public function index(){
        ...
        $this->load->view('transaksi1/produksi/work_order/list_view', $object);
    }
...
>>>>>>> 8281b7891b2d52ae86f2a0749f32dd848350def3
				...
				if($object['data']['status'] == 1){
					if($getlocked[0]['U_Locked'] != 'N' || $ucaneditqty[0]['CanEditQty'] != 'N'){
						$getucaneditqty = '<input type="text" id="editqty_'.$i.'" class="form-control" value="'.$data['qty'].'">';
					}else {
						$getucaneditqty = '<input type="text" id="editqty_'.$i.'" class="form-control" value="'.$data['qty'].'" readonly>';
					}
				}else{
					$getucaneditqty = '<input type="text" id="editqty_'.$i.'" class="form-control" value="'.$data['qty'].'" readonly>';
				}
